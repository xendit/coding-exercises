...
describe('csv processor tests', () => {
    ...
    describe('control flow', () => {
        it('should return error when paths to files are not an array', (done) => {
            const pathsToFiles = 'foo';
            const pathToOutput = `${__dirname}/output/hello.csv`;
            const headers = ['foo', 'bar'];
            const stubbedFS = {};

            CSVProcessor.controlFlow(stubbedFS, pathsToFiles, pathToOutput, headers, (err) => {
                expect(err).to.be.an('error');
        ...
        it('should return error when reading file', (done) => {
            const pathsToFiles = ['foo.csv'];
            const pathToOutput = 'foo.csv';
            const headers = ['foo', 'bar'];
            const stubbedFS = {
        ...
        it('should build combined file', (done) => {
            const pathsToFiles = ['foo.csv'];
            const pathToOutput = 'foo.csv';
            const headers = ['foo', 'bar'];
            const stubbedFS = {
            ...
            CSVProcessor.controlFlow(stubbedFS, pathsToFiles, pathToOutput, headers, done);
        });
    });
});
